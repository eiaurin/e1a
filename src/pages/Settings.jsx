        ...
        <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-xl shadow divide-y ${darkMode ? 'divide-gray-700' : 'divide-gray-100'}`}>
          <div className="p-5 flex items-center justify-between">
            ...
            <button
              ...
              className={`w-16 h-8 rounded-full transition-colors relative select-none flex items-center ${soundEnabled ? t.toggleActive : 'bg-gray-300'}`}
            >
              <span className={`w-7 h-7 bg-white rounded-full shadow transition-transform select-none ${soundEnabled ? 'translate-x-8' : 'translate-x-0.5'}`} />
            </button>
          </div>
          ...
          <div className="p-5 flex items-center justify-between">
            ...
            <button
              ...
              className={`w-16 h-8 rounded-full transition-colors relative select-none flex items-center ${darkMode ? t.toggleActive : 'bg-gray-300'}`}
            >
              <span className={`w-7 h-7 bg-white rounded-full shadow transition-transform select-none ${darkMode ? 'translate-x-8' : 'translate-x-0.5'}`} />
            </button>
          </div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
              {Object.entries(themes).map(([key, th]) => (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 p-2 rounded-xl border-2 transition-all select-none ${
                    theme === key
          ...
          <div className="p-5">
            ...
            {showConfirm && (
              <div className={`mt-2 p-3 ${t.resetWarning} rounded-lg text-sm`}>
                ...
                <button onClick={() => setShowConfirm(false)} className="underline select-none">Cancel</button>
              </div>
            )}
          </div>
        </div>
